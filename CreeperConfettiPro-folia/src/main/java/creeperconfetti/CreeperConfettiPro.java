...
public class CreeperConfettiPro extends JavaPlugin {
    ...
    // Cached so region threads (Folia) never read the non thread-safe config.
    private volatile double confettiChance = 100.0D;
    ...
    /**
     * Confetti chance in percent. Cached in a volatile field because creepers
     * can explode on any region thread while the config is being written.
     */
    public double getConfettiChance() {
        return confettiChance;
    }

    public void setConfettiChance(double value) {
        this.confettiChance = value;
    }
    ...
    /**
     * Refreshes the values that region threads read into volatile cache fields.
     */
    public void reloadCachedSettings() {
        this.confettiChance = getConfig().getDouble("confetti_chance", 100.0D);
